...
def FillData(df):
    ...
    df.loc[(df["occupation"] == "4") | (df["occupation"] == "SCHOOL") | (df["occupation"] == "EX TEACHER") | (
            df["occupation"] == "4/E") | (df["occupation"] == "TEACHER") | (df["occupation"] == "BLIND TEACHER") | (
                   ...
                   df["occupation"] == "PRINCIPAL") | (
                       df["occupation"] == "PROFFESSOR"), "occupation"] = "TEACHING PROFESSIONALS"
    ...
    # education filter
    df['edu'] = df['edu'].replace(['IL', 'P', 'S', 'H', 'G', 'PG', 'O', 'SG', 'M', 'P', 'P '],
    ...
    # vehicle
    df['vehicle'] = (1 * df['4_wheeler']) + (0.75 * df['2_wheeler']) + (0.5 * df['cycle'])
    ...
    lu=pd.read_csv(r'F:\IIT KGP\III Semester\Thesis\Research\Raw Data\LU.csv')
    '''
    for i in df.index:
        for j in range (6):
            if(df['Id'][j])==i:
                if(df['Landuse_AN'][j])=='R':
                    rw=df['pp'][j]

    if
    # random.choices(list, weights=(rw,cw,sw,ow), k=1)
    '''
    return df